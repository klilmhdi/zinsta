...
import 'package:zinsta/components/profile_components/profile_details.dart';
import 'package:zinsta/screens/layout/profile/edit_profile_screen.dart';
import 'package:zinsta/screens/layout/settings/settings_layout.dart';

import '../../../blocs/post_blocs/fetch_user_posts_bloc/fetch_user_posts_bloc.dart';
import '../../../components/post_components/empty_post_message.dart';
import '../../../components/post_components/post_widget.dart';
...
Widget buildProfileContent(BuildContext context, MyUser user, {required bool isCurrentUser}) {
  final followersBloc = context.read<FollowersBloc>();
  final followersState = followersBloc.state;

  if (followersState is! FollowersLoaded && followersState.targetUserId != user.id) {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<FollowersBloc>().add(LoadFollowersCount(user.id));
    });
  }
  final followersCount = followersState is FollowersLoaded ? followersState.followersCount : 0;
  final followingCount = followersState is FollowersLoaded ? followersState.followingCount : 0;
                ...
                children: [
                  ...
                  if (isCurrentUser)
                    ProfileActionButtons(
                      onSettingsPressed:
                          () => Animations().rtlNavigationAnimation(context, SettingLayout()),
                      onEditProfilePressed:
                          () => Animations().rtlNavigationAnimation(context, EditProfileScreen()),
                    ),
                ],
              ),
            ),
...
//           (_) => context.read<FollowersBloc>().add(LoadFollowersCount(user.id)),
//         );
//       }
//       final followersCount = followersState is FollowersLoaded ? followersState.followersCount : 0;
//       final followingCount = followersState is FollowersLoaded ? followersState.followingCount : 0;
...
//                       ),
//                     ),
//                   if (state.status == UserPostsStatus.success && state.posts.isEmpty)
//                     SliverToBoxAdapter(
//                       child: SizedBox(
//                         height: MediaQuery.of(context).size.height * 0.35,
//                         child: emptyPostsWidget(),
//                       ),
//                     ),
//                   if (state.status == UserPostsStatus.success && state.posts.isNotEmpty)
//                     SliverList(
//                       delegate: SliverChildBuilderDelegate((context, index) {
