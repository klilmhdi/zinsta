...
class FirebaseUserRepository implements UserRepository {
  ...
  Future<List<MyUser>> searchUserFromUsername(String query) async {
    ...
    // Search by name
    final nameSnapshot = await usersCollection
        .where('name', isGreaterThanOrEqualTo: lowerQuery)
        .where('name', isLessThan: '${lowerQuery}z')
        .get();

    // Search by username
    final usernameSnapshot = await usersCollection
  ...
  //     return MyUser.fromEntity(MyUserEntity.fromDocument(value.data() as Map<String, dynamic>));
  //   }).toList();
  // }
  ...
  Future<bool> isFollowing(String currentUserId, String targetUserId) async {
    final doc =
        await usersCollection.doc(currentUserId).collection('following').doc(targetUserId).get();
    return doc.exists;
  }
